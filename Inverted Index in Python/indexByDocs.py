# -------------------------------------------------------------------------------
# Name:        indexByDocs
# Purpose:     index by no of documents
#
...
def writeInvIndex(temp_ind, id):
    ...
    index.close()
    catalog.close()
    print "Index", id, "written to disk."
...
def getInvListFromOffsets(offsets, id):
    with open(os.path.join(constants.TEMP_DIR, \
                           'index_set_' + str(id) + '.dat'), "r") as f:
        f.seek(offsets[0])
